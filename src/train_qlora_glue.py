...
def keep_classifier_fp32(model: nn.Module):
    """
    Keep classifier head (and pooler.dense) in fp32 even if encoder is 4-bit.
    Safe to call before and after prepare_model_for_kbit_training.
    """
    try:
    ...
    def _upgrade_path(module_path: str):
        # Walk to the submodule and if it's a 4-bit Linear, replace with nn.Linear fp32.
        mod = model
        ...
        if isinstance(mod, Linear4bit):
            ...
            use_bias = getattr(mod, "bias", None) is not None
            new_lin = nn.Linear(in_f, out_f, bias=use_bias).to(torch.float32)
            try:
                with torch.no_grad():
    ...
    # Try common heads
    _upgrade_path("classifier")
    _upgrade_path("classifier.dense")
    _upgrade_path("classifier.out_proj")
    _upgrade_path("pooler.dense")

    # Ensure requires_grad and dtype for pooler/classifier if they exist
    ...
    if pooler is not None and hasattr(pooler, "dense"):
        try:
            pooler.dense.to(torch.float32)
            for p in pooler.dense.parameters():
                p.requires_grad = True
        except Exception:
            pass
